...
class EditPeopleForm(FlaskForm):
    ...
    gender = StringField(u"性别",
                validators=[DataRequired(message=u"该项忘了填写了!"), Length(1, 128, message=u"长度为1到128个字符")])
    id_type = StringField(u"证件类型",
                validators=[DataRequired(message=u"该项忘了填写了!"), Length(1, 128, message=u"长度为1到128个字符")])
    id_number = StringField(u"证件号码",
                validators=[DataRequired(message=u"该项忘了填写了!"), Length(1, 128, message=u"长度为1到128个字符")])
    phone_number = StringField(u"手机号码",
                validators=[DataRequired(message=u"该项忘了填写了!"), Length(1, 128, message=u"长度为1到128个字符")])
    address = StringField(u"地址", validators=[Length(0, 128, message=u"长度为0到128个字符")])
    submit = SubmitField(u"保存更改")
...
class AddPeopleForm(EditPeopleForm):
    def validate_id_number(self, filed):
        if Residents.query.filter_by(id_number=filed.data).count():
            raise ValidationError(u'已经存在相同的证件号码,无法录入,请仔细核对是否已登记该人员信息.')
